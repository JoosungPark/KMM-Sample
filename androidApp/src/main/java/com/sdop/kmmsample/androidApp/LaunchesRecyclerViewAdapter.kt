...
class LaunchesRecyclerViewAdapter(var launches: List<RocketLaunch>) : RecyclerView.Adapter<LaunchesRecyclerViewAdapter.LaunchViewHolder>() {
    ...
    inner class LaunchViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(launch: RocketLaunch) {
            ...
            if (launchSuccess != null ) {
                if (launchSuccess) {
                    launchSuccessTextView.text = context.getString(R.string.successful)
                    launchSuccessTextView.setTextColor((ContextCompat.getColor(itemView.context, R.color.colorSuccessful)))
                } else {
                    launchSuccessTextView.text = context.getString(R.string.unsuccessful)
